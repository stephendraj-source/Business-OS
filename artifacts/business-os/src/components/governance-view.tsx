...
type PendingGovernanceFile = { id: string; file: File };
...
export function GovernanceView() {
  ...
  const [pendingFiles, setPendingFiles] = useState<PendingGovernanceFile[]>([]);
  ...
  function appendPendingFiles(files: FileList | File[]) {
  ...
  async function saveEdit() {
    ...
    try {
      if (editingId === 'new') {
        ...
        const created = await res.json() as GovernanceStandard;
        if (pendingFiles.length > 0) {
          setUploadingFor(created.id);
          const formData = new FormData();
          pendingFiles.forEach(({ file }) => formData.append('files', file));
          const { 'Content-Type': _ct, ...uploadHeaders } = fetchHeaders();
          await fetch(`${API}/governance/${created.id}/documents`, {
            method: 'POST',
        ...
        {editingId === 'new' && (
          <div className="rounded-xl border-2 border-primary/30 bg-primary/5 p-4">
            ...
            <EditForm
              ...
              onPendingFilesAdd={appendPendingFiles}
...
function EditForm({
  ...
  pendingFiles?: PendingGovernanceFile[];
  onPendingFilesAdd?: (files: FileList | File[]) => void;
  onPendingFileRemove?: (id: string) => void;
}) {
  const canAttachFiles = !!onPendingFilesAdd && !!onPendingFileRemove;
    ...
    <div className="space-y-3">
      ...
      {canAttachFiles && (
        <div className="space-y-2">
          <div>
            <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider">Supporting Files</label>
            <label
              onDragOver={e => {
                e.preventDefault();
                setIsDragOver(true);
              ...
              onDrop={e => {
                ...
                onPendingFilesAdd!(e.dataTransfer.files);
              }}
              className={cn(
                "mt-1 flex cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border border-dashed px-4 py-5 text-center transition-all",
                isDragOver
            ...
            >
              ...
              <input
                ...
                onChange={e => {
                  ...
                  onPendingFilesAdd!(e.target.files);
            ...
            <p className="mt-1 text-[10px] text-muted-foreground/70">
              Add one or more files now and they will upload automatically when you save the standard.
            </p>
          </div>

          {pendingFiles && pendingFiles.length > 0 && (
            <div className="rounded-lg border border-border bg-background/70 p-2 space-y-1.5">
              <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                Selected Files ({pendingFiles.length})
              </div>
              {pendingFiles.map(({ id, file }) => (
                <div key={id} className="flex items-center justify-between gap-2 rounded-md bg-secondary/40 px-2 py-1.5">
                  ...
                  <button
                    type="button"
                    onClick={() => onPendingFileRemove!(id)}
                    className="flex items-center gap-1 rounded-md border border-border px-2 py-1 text-[10px] text-muted-foreground hover:bg-secondary transition-colors"
                  >
                    <X className="w-3 h-3" />
                    Remove
                  </button>
