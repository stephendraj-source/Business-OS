...
import { X, Send, Plus, Trash2, ChevronDown, Bot, Loader2, Sparkles, GripHorizontal, MessageSquare, Minus, ExternalLink, Wrench, CheckCircle2, XCircle, Layers, ArrowRight, Hash, GitBranch, User, ClipboardList, AlertCircle, Calendar, Maximize2, Minimize2 } from 'lucide-react';
...
export function Chatbot() {
  ...
  const loadConversation = async (id: number) => {
    try {
      ...
      setShowConvList(false); setPendingTasks([]); setDismissedTaskIds([]);
    } catch {}
  };
        ...
        for (const line of lines) {
          ...
          try {
            ...
            if (parsed.tool_result) {
              ...
              if (parsed.tool_result.success && parsed.tool_result.data?.task_id) {
                setPendingTasks(prev => {
                  const task = parsed.tool_result.data as PendingTask;
                  if (prev.some(existing => existing.task_id === task.task_id)) return prev;
                  return [...prev, task];
                });
              }
              // Open task quick-create form
              if (parsed.tool_result.name === 'suggest_task' && parsed.tool_result.success && parsed.tool_result.data && !parsed.tool_result.data?.task_id) {
  ...
  }, [input, streaming, activeConvId, fetchHeaders]);
  ...
  const startNewChat = () => {
    ...
    setShowConvList(false); setInput(''); setPendingTasks([]); setDismissedTaskIds([]);
    setTimeout(() => inputRef.current?.focus(), 50);
  };
      ...
      {open && (
        <div
          ...
          style={{
            ...
            width: isFullscreen ? '100vw' : PANEL_W,
            height: isMinimized ? 'auto' : isFullscreen ? '100vh' : PANEL_H,
            zIndex: isFullscreen ? 80 : 50,
            userSelect: isDragging ? 'none' : undefined,
            transition: 'height 0.2s ease, width 0.2s ease, left 0.2s ease, top 0.2s ease, right 0.2s ease, bottom 0.2s ease',
          }}
          className={cn(
            "bg-card shadow-2xl flex flex-col overflow-hidden",
            isFullscreen ? "rounded-none border-0" : "rounded-2xl border border-border"
          )}
        >
          {/* Drag handle header */}
          <div
          ...
          >
            ...
            <div className="flex items-center gap-1">
              {!isFullscreen && <GripHorizontal className="w-4 h-4 text-muted-foreground/30 mr-1 pointer-events-none" />}
              <button
              ...
              <button
                onClick={() => { setOpen(false); setIsMinimized(false); setIsFullscreen(false); }}
                className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-secondary transition-colors"
                title="Close"
              >
                <X className="w-4 h-4" />
          ...
          <div className={cn("flex flex-col flex-1 overflow-hidden", isMinimized && "hidden")}>
            ...
            {showConvList && (
              <div className="border-b border-border bg-sidebar flex-none">
                ...
                <div className="overflow-y-auto" style={{ maxHeight: isFullscreen ? '180px' : '120px' }}>
                    ...
                    {(assistant || isCurrentlyStreaming) && (
                      <div className="flex items-start gap-2 pl-2">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {isLast && !streaming && pendingTasks
                            .filter(task => !dismissedTaskIds.includes(task.task_id))
                            .map(task => (
                              <QueuePicker
                                key={task.task_id}
                                task={task}
                                queues={queues}
                                onSelect={(queueId, queueName) => assignQueue(task.task_id, queueId, queueName)}
                                onDismiss={() => setDismissedTaskIds(prev => [...prev, task.task_id])}
                              />
                            ))}
