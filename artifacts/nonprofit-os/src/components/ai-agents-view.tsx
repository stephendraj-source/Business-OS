...
import {
  ...
  ToggleLeft, ToggleRight, Edit2, Save, Hash, Wrench,
} from "lucide-react";
...
function InstructionsEditor({
  ...
  const insertField = (field: ProcessField) => {
    const ta = textareaRef.current;
    if (!ta) return;
    ...
    const newVal = `${before}{{${field.key}}}${after}`;
    onChange(newVal);
    setShowPicker(false);
    setTimeout(() => {
      ta.setSelectionRange(before.length + field.key.length + 4, before.length + field.key.length + 4);
      ta.focus();
    }, 0);
  };
  ...
  return (
    <div className="relative">
      <textarea
        ...
        rows={8}
        placeholder={`Write the agent's instructions here.\nType / to insert a database field placeholder, e.g. {{processName}}...`}
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <span className="text-xs text-muted-foreground">Type <kbd className="px-1 py-0.5 bg-secondary rounded text-xs font-mono">/</kbd> to insert a process field placeholder</span>
      </div>
      {showPicker && (
        <div
          className="fixed z-[99] w-72 bg-popover border border-border rounded-xl shadow-2xl overflow-hidden"
        ...
        >
          ...
          <div className="max-h-48 overflow-y-auto">
            {filteredFields.length === 0 ? (
              <div className="px-3 py-3 text-xs text-muted-foreground text-center">No fields match</div>
...
export function AiAgentsView() {
  ...
  useEffect(() => { fetchAgents(); fetchFields(); }, [fetchAgents, fetchFields]);
