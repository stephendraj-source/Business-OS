...
export function TaskCreatePanel({ nodeTopic, onClose, onCreated }: Props) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      // Parse assignedTo: "user:123" or "agent:456"
      ...
      setDone(true);
      setTimeout(() => onCreated(task.id), 900);
    } catch {
      setError('Could not create task. Please try again.');
    } finally {
      setSaving(false);
    }
  }, [name, description, priority, startDate, endDate, assignedTo, queueId, fetchHeaders, onCreated]);
      ...
      >
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            {queues.length > 0 && (
              <div className="space-y-1">
                <label htmlFor="tcp-queue" className="text-xs font-medium text-muted-foreground">Queue</label>
                <select
                  id="tcp-queue"
                  value={queueId}
