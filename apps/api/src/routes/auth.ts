...
async function resolveRegistrationTenantId(): Promise<number | null> {
...
authRouter.post('/register', async (req, res) => {
  try {
    ...
    const tenantId = await resolveRegistrationTenantId();
    if (tenantId === null) {
      return res.status(503).json({ error: 'Registration is unavailable because no tenant exists yet.' });
    }
