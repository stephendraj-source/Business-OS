  ...
  try {
    ...
    children.push(
      ...
      makeTable(
        ...
        [
          ["apps/api",                  "@workspace/api-server",        "Express REST API, all business logic"],
          ["apps/web",                  "@workspace/business-os",       "React + Vite frontend SPA"],
          ["packages/db",               "@workspace/db",                "Drizzle schema + DB connection pool"],
          ["packages/api-zod",          "@workspace/api-zod",           "Generated Zod schemas from OpenAPI"],
          ["packages/api-client-react", "@workspace/api-client-react",  "Generated React Query hooks (Orval)"],
          ["tools/scripts",             "@workspace/scripts",           "Seed script (100 processes)"],
        ],
        [3000, 2800, 3200]
      ),
      ...spacer(2),
    ...
    children.push(
      ...
      body("The frontend is a React 18 SPA built with Vite. It lives at apps/web. Routing is state-based (no URL router) — a single activeView state drives which view component renders inside the Layout shell."),
      ...spacer(1),
        ...
        stories: [
          ...
          },
          {
            ...
            ac: [
              ...
              "Seed script (tools/scripts/src/seed.ts) seeds 100 nonprofit processes with pnpm --filter @workspace/scripts run seed",
            ],
          },
        ],
      },
    ...
    children.push(
      ...
      bullet("Create a pnpm workspace with pnpm-workspace.yaml listing apps/*, packages/*, and tools/scripts"),
      ...
      bullet("src/app/providers/AuthContext.tsx: read/write JWT from localStorage key business-os-auth-token; useAuth() and useUser() hooks"),
      bullet("src/shared/components/layout.tsx: defines ActiveView union type; SECTIONS_DEF; ITEMS_DEF (all nav items with sectionId); VIEW_META (label + section per view); getIcon() switch; drag-reorder sidebar; breadcrumb header"),
      bullet("src/app/routes/dashboard.tsx: useState<ActiveView>; renders correct view component inside AnimatePresence"),
