...
import { BUSINESS_OS_TOKEN_KEY, LEGACY_NONPROFIT_OS_TOKEN_KEY, getStoredValue } from '@/shared/lib/storage';
  ...
  const importFile = async (file: File) => {
    ...
    try {
      ...
      const parseRes = await fetch(`${API}/meetings/parse-file`, {
        method: 'POST',
        headers: (() => { const t = getStoredValue(BUSINESS_OS_TOKEN_KEY, LEGACY_NONPROFIT_OS_TOKEN_KEY); return t ? { Authorization: `Bearer ${t}` } : {}; })(),
        body: formData,
      });
...
function MeetingDetail({ meeting, processes, workflows, agents, users, onUpdate }: DetailProps) {
  ...
  const [meetingType, setMeetingType] = useState<MeetingType>(meeting.meeting_type ?? 'physical');
  ...
  useEffect(() => {
    ...
    setMeetingType(meeting.meeting_type ?? 'physical');
    setMeetingDate(meeting.meeting_date ? meeting.meeting_date.slice(0, 16) : '');
    setLocation(meeting.location ?? '');
    setOrganizerName(meeting.organizer_name || meeting.organizer_user_name || '');
    setOrganizerId(meeting.organizer_id);
    setProcessId(meeting.process_id);
    setDiscussions(meeting.discussions ?? '');
    setAgenda(parseSafe(meeting.agenda ?? '[]', []));
    setAttendees(parseSafe(meeting.attendees ?? '[]', []));
    setActions(parseSafe(meeting.actions ?? '[]', []));
    setLinkedWfIds((meeting.linked_workflows ?? []).map(w => w.workflow_id));
    setLinkedAgentIds((meeting.linked_agents ?? []).map(a => a.agent_id));
  }, [meeting]);
...
function DiscussionsTab({ value, onChange }: { value: string | null | undefined; onChange: (v: string) => void }) {
  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      <textarea
        value={safeValue} onChange={e => onChange(e.target.value)}
      ...
      />
      <p className="text-[11px] text-muted-foreground mt-1.5">{safeValue.length.toLocaleString()} characters · auto-saved</p>
    </div>
  );
}
