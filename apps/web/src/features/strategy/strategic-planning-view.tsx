    ...
    <div className="flex-1 min-w-0 flex flex-col border-l border-border bg-card/40 h-full">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Measure Name</label>
          <input
            value={form.name}
        ...
        <div className="space-y-1.5">
          <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Actual KPI Result</label>
          <textarea
            rows={4}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="flex max-h-[calc(100vh-2rem)] w-[760px] max-w-[calc(100vw-2rem)] flex-col rounded-2xl border border-border bg-card shadow-2xl">
        <div className="flex items-center justify-between px-6 py-5">
          <div>
            <h2 className="text-lg font-semibold">New Measure</h2>
            <p className="text-xs text-muted-foreground mt-0.5">Create a measure directly from Strategic Planning.</p>
          </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <div className="space-y-2 pt-2 border-t border-border">
          ...
          {allInitiatives.length === 0 ? (
            <p className="text-xs text-muted-foreground italic">No measures available.</p>
          ) : (
...
export function StrategicPlanningView() {
  ...
  const tabs: { key: PlanningTab; label: string; icon: React.ElementType; desc: string }[] = [
    ...
    { key: 'initiatives', label: 'Measures',          icon: Lightbulb, desc: 'Measures linked to processes and teams' },
  ];
