...
function ini_goal_id(ini: InitiativeRow) { return ini.goalId ?? ini.goal_id ?? null; }
function ini_goal_title(ini: InitiativeRow) { return ini.goalTitle ?? ini.goal_title ?? null; }
function ini_goal_color(ini: InitiativeRow) { return ini.goalColor ?? ini.goal_color ?? null; }
function ini_goal_number(ini: InitiativeRow) { return ini.goalNumber ?? ini.goal_number ?? null; }
...
export function InitiativesView() {
  ...
  for (const ini of filtered) {
    const key = ini_goal_id(ini);
    if (!seen.has(key)) {
      seen.set(key, groupedByGoal.length);
      groupedByGoal.push({ goalId: key, goalTitle: ini_goal_title(ini), goalColor: ini_goal_color(ini), goalNumber: ini_goal_number(ini), items: [] });
    }
          ...
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            ...
            {ini_goal_id(initiative) && (
              <span className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full font-semibold bg-secondary text-muted-foreground">
                <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: ini_goal_color(initiative) ?? '#6366f1' }} />
                {ini_goal_title(initiative)}
              </span>
            )}
          </div>
