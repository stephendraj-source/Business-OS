...
import { useProcessesData, useCategoriesData, useOptimisticUpdateProcess, useDeleteProcessRow, useCreateProcessMutation, useAiPopulateProcessMutation } from '@/shared/hooks/use-app-data';
...
import { useFavourites } from '@/app/providers/FavouritesContext';
...
function bpmnModelerHref(processId: number) {
  return `/?view=operaton&processId=${processId}`;
}

function bpmnLinkLabel(xml: string | null | undefined, fallback: string) {
  if (xml) {
    const match = xml.match(/<bpmn:process\b[^>]*\bname="([^"]+)"/i);
...
function ProcessDetailPanel({ process: initialProcess, onClose }: { process: Process; onClose: () => void }) {
  ...
  const availableParentProcesses = useMemo(
    () => (processes ?? [])
  ...
  function save(field: string, value: string | boolean | number | null) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background flex flex-col">
        ...
        <div className="max-w-6xl mx-auto px-8 py-6 grid grid-cols-[340px_1fr] gap-8 items-start">
          ...
          <div>
            <div className="text-[10px] font-semibold text-muted-foreground/70 uppercase tracking-wider mb-1">Subprocess Of</div>
            <select
              value={process.parentProcessId == null ? '' : String(process.parentProcessId)}
              onChange={e => save('parentProcessId', e.target.value ? Number(e.target.value) : null)}
              className="w-full px-3 py-2 text-sm border border-border/50 bg-secondary/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/40 cursor-pointer hover:border-primary/30 hover:bg-secondary/50 transition-all"
            >
              <option value="">No parent process</option>
              {availableParentProcesses.map(parent => (
                <option key={parent.id} value={String(parent.id)}>
                  {`PRO-${String(parent.number).padStart(3, '0')} ${parent.processName || parent.processDescription}`}
                </option>
              ))}
            </select>
          </div>
...
export function ProcessTable({ mode = 'matrix' }: TableProps) {
  ...
  const [subprocessParent, setSubprocessParent] = useState<Process | null>(null);
  ...
  const pinnedLeftOffsets = useMemo(() => {
    const keysToPin = allVisibleCols
      .map(col => col.key)
      .slice(0, Math.max(allVisibleCols.findIndex(col => col.key === 'processName') + 1, 0));
    let left = 0;
    ...
    return offsets;
  }, [allVisibleCols, widths]);

  function isPinnedLeftColumn(key: string) {
    return Object.prototype.hasOwnProperty.call(pinnedLeftOffsets, key);
  }
  ...
  function getPinnedCellStyle(key: string, width: number | undefined, isHeader = false): React.CSSProperties {
    const base: React.CSSProperties = { width };
    if (!isPinnedLeftColumn(key)) {
      return base;
    }
    return {
      ...base,
      position: 'sticky',
      left: pinnedLeftOffsets[key] ?? 0,
  ...
  const compareProcesses = useCallback((a: Process, b: Process) => {
  ...
  }, [processes, search, selectedCategory, mode, compareProcesses]);

  const childProcessCounts = useMemo(() => {
    const counts: Record<number, number> = {};
    const visibleIds = new Set(filteredProcesses.map(p => p.id));
  ...
  useEffect(() => {
    setExpandedProcessIds(prev => {
      const next = { ...prev };
      Object.keys(childProcessCounts).forEach(key => {
        const id = Number(key);
  ...
  const visibleProcessRows = useMemo<ProcessTreeRow[]>(() => {
    const visibleIds = new Set(filteredProcesses.map(p => p.id));
    const byParent = new Map<number | null, Process[]>();
    const compareWithinBranch = (a: Process, b: Process) => {
      if (sortKey && sortKey !== 'category') {
    ...
    for (const process of filteredProcesses) {
      const parentId = process.parentProcessId != null && visibleIds.has(process.parentProcessId)
    ...
    const walk = (parentId: number | null, depth: number) => {
      const children = (byParent.get(parentId) ?? []).slice().sort(parentId == null ? compareProcesses : compareWithinBranch);
      children.forEach(process => {
        rows.push({ process, depth });
        if ((expandedProcessIds[process.id] ?? true) && (byParent.get(process.id)?.length ?? 0) > 0) {
    ...
    walk(null, 0);
    return rows;
  }, [filteredProcesses, expandedProcessIds, compareProcesses, sortKey]);
  ...
  function renderCell(process: Process, colKey: string, row?: ProcessTreeRow) {
    const depth = row?.depth ?? 0;
    const isSubprocess = depth > 0;
    const subprocessDisplayClass = isSubprocess ? "italic text-muted-foreground" : undefined;
    switch (colKey) {
      case 'include':
        return (
          <td key="include" className={cn("align-middle p-0 text-center", getPinnedCellClass('include'))} style={getPinnedCellStyle('include', widths['include'])}>
      ...
      case '#':
        return (
          <td key="#" className={cn("align-middle p-0 text-center overflow-visible", getPinnedCellClass('#'))} style={getPinnedCellStyle('#', widths['#'])}>
            <div className="flex flex-col items-center justify-center gap-1 py-2 px-2" style={{ paddingLeft: `${depth * 18}px` }}>
              <div className="flex items-center gap-1">
                {(childProcessCounts[process.id] ?? 0) > 0 ? (
                  <button
                    className="inline-flex h-5 w-5 items-center justify-center rounded border border-border/50 bg-background text-[12px] font-bold text-muted-foreground transition-colors hover:border-primary/30 hover:text-primary"
                    onClick={(event) => {
                      event.preventDefault();
                      event.stopPropagation();
                      setExpandedProcessIds(prev => ({ ...prev, [process.id]: !(prev[process.id] ?? true) }));
                    }}
                    title={(expandedProcessIds[process.id] ?? true) ? 'Collapse subprocesses' : 'Expand subprocesses'}
                  >
                    {(expandedProcessIds[process.id] ?? true) ? '−' : '+'}
                  </button>
                ) : (
                  <span className="inline-flex h-5 w-5 items-center justify-center text-muted-foreground/35">
                    {isSubprocess ? <span className="text-[13px] font-semibold leading-none text-muted-foreground/60">↳</span> : null}
                  </span>
                )}
                <button
              ...
              <button
                onClick={() => { setSubprocessParent(process); setShowAddModal(true); }}
                className="rounded border border-border/60 px-1.5 py-0.5 text-[10px] font-medium text-muted-foreground transition-colors hover:border-primary/30 hover:bg-primary/5 hover:text-primary"
      ...
      case 'category':
        return (
          <td key="category" className={cn("align-middle p-3 overflow-hidden", getPinnedCellClass('category'))} style={getPinnedCellStyle('category', widths['category'])}>
            <span className={cn(
      ...
      case 'processName':
        return (
          <td key="processName" className={cn("overflow-hidden p-0", getPinnedCellClass('processName'))} style={getPinnedCellStyle('processName', widths['processName'])}>
            <button
              className={cn(
                "w-full text-left px-3 py-3 text-sm font-medium text-foreground hover:text-primary block transition-colors group whitespace-normal break-words",
                isSubprocess && "italic text-muted-foreground"
              )}
              onClick={() => setDetailProcess(process)}
              title="Click to view process details"
              style={isSubprocess ? { paddingLeft: `${12 + depth * 18}px` } : undefined}
            >
              <span className="flex items-start gap-2 group-hover:underline underline-offset-2 whitespace-normal break-words">
                {isSubprocess ? (
                  <span className="mt-0.5 shrink-0 text-[13px] font-semibold leading-none text-muted-foreground/70 not-italic">↳</span>
                ) : null}
      ...
      case 'aiAgent':
        return (
          <td key="aiAgent" className="overflow-hidden p-0" style={{ width: widths['aiAgent'] }}>
            <EditableCell processId={process.id} field="aiAgent" initialValue={process.aiAgent} onSaved={cellSaved(process, 'aiAgent')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'aiAgentActive':
        return (
      ...
      case 'purpose':
        return (
          <td key="purpose" className="overflow-hidden p-0" style={{ width: widths['purpose'] }}>
            <EditableCell processId={process.id} field="purpose" initialValue={process.purpose} multiline onSaved={cellSaved(process, 'purpose')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'inputs':
        return (
          <td key="inputs" className="overflow-hidden p-0" style={{ width: widths['inputs'] }}>
            <EditableCell processId={process.id} field="inputs" initialValue={process.inputs} multiline onSaved={cellSaved(process, 'inputs')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'outputs':
        return (
      ...
      case 'kpi':
        return (
          <td key="kpi" className="overflow-hidden p-0" style={{ width: widths['kpi'] }}>
            <EditableCell processId={process.id} field="kpi" initialValue={process.kpi} multiline onSaved={cellSaved(process, 'kpi')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'bpmn':
        return (
      ...
      case 'target':
        return (
          <td key="target" className="overflow-hidden p-0" style={{ width: widths['target'] }}>
            <EditableCell processId={process.id} field="target" initialValue={process.target} multiline onSaved={cellSaved(process, 'target')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'achievement':
        return (
          <td key="achievement" className="overflow-hidden p-0" style={{ width: widths['achievement'] }}>
            <EditableCell processId={process.id} field="achievement" initialValue={process.achievement} multiline onSaved={cellSaved(process, 'achievement')} displayClassName={subprocessDisplayClass} />
          </td>
        );
      case 'priority': {
        const pv = (process as any).priority as number | null ?? 5;
        return (
          <td key="priority" className={cn("align-middle p-0", getPinnedCellClass('priority'))} style={getPinnedCellStyle('priority', widths['priority'])}>
      ...
      case 'estimatedValueImpact':
        return (
          <td key="estimatedValueImpact" className="overflow-hidden p-0" style={{ width: widths['estimatedValueImpact'] }}>
            <EditableCell processId={process.id} field="estimatedValueImpact" initialValue={process.estimatedValueImpact} multiline onSaved={cellSaved(process, 'estimatedValueImpact')} displayClassName={subprocessDisplayClass} />
          </td>
        );
  ...
  return (
    <div className="h-full flex flex-col bg-background relative overflow-hidden">
      ...
      <div className="flex-1 overflow-auto bg-card">
        <table
          className="spreadsheet-table border-separate border-spacing-0"
                ...
                return (
                  <th
                    ...
                    className={cn(
                      ...
                      getPinnedCellClass(col.key, true),
                    ...
                    style={getPinnedCellStyle(col.key, widths[col.key] ?? col.defaultWidth, true)}
          ...
          <tbody>
            {visibleProcessRows.length === 0 ? (
              ...
              (() => {
                ...
                let lastCat = '';
                visibleProcessRows.forEach(row => {
                  const process = row.process;
                  if (!sortKey && row.depth === 0 && process.category !== lastCat) {
                    lastCat = process.category;
                    rows.push(
                      <tr key={`cat-header-${process.category}`} className="select-none">
                        <td colSpan={allVisibleCols.length} className="px-0 py-0 bg-secondary/40 border-y border-border/60">
                          <div className="sticky left-0 z-20 w-fit px-4 py-2 bg-secondary/40 border-r border-border/60">
                            <span className={cn(
                  ...
                  rows.push(
                    <tr
                      ...
                      className={cn(
                        "bg-transparent",
                        row.depth > 0 && process.included && mode === 'matrix' && "[&>td]:bg-primary/[0.03]"
                      )}
                    >
                      {allVisibleCols.map(col => renderCell(process, col.key, row))}
                    </tr>
                  );
                });
                return rows;
      ...
      {showAddModal && (
        <AddProcessModal
          ...
          processes={processes ?? []}
          initialParentProcess={subprocessParent}
          onClose={() => { setShowAddModal(false); setSubprocessParent(null); }}
          onCreateAndPopulate={(body, useAi) => {
            if (subprocessParent) {
              setExpandedProcessIds(prev => ({ ...prev, [subprocessParent.id]: true }));
            }
            createProcess({ data: body as any }, {
              onSuccess: (created) => {
                ...
                if (useAi) {
                  aiPopulate({ id: created.id }, { onSettled: () => { setShowAddModal(false); setSubprocessParent(null); } });
                } else {
...
function AddProcessModal({
  ...
  categories: string[];
  processes: Process[];
  initialParentProcess: Process | null;
  onClose: () => void;
  onCreateAndPopulate: (body: Record<string, string | boolean | number | null>, useAi: boolean) => void;
  isCreating: boolean;
  ...
  const [included, setIncluded]                     = useState(initialParentProcess?.included ?? false);
  const [parentProcessId, setParentProcessId]       = useState(initialParentProcess ? String(initialParentProcess.id) : '');
  const [isDraftFilling, setIsDraftFilling]         = useState(false);

  useEffect(() => {
    setParentProcessId(initialParentProcess ? String(initialParentProcess.id) : '');
    if (initialParentProcess?.category) {
      setCategory(initialParentProcess.category);
    }
    setIncluded(initialParentProcess?.included ?? false);
  }, [initialParentProcess]);

  const isBusy = isCreating || isPopulating || isDraftFilling;
  ...
  const submitProcess = (useAi: boolean) => {
  ...
  const handleDraftAiFill = async () => {
    if (!category || !processDescription.trim()) return;
    setIsDraftFilling(true);
    try {
      const response = await fetch('/api/processes/ai-draft', {
        method: 'POST',
        headers: { ...fetchHeaders(), 'Content-Type': 'application/json' },
        body: JSON.stringify({
          category,
      ...
      if (!target && payload.target) setTarget(payload.target);
      if (!achievement && payload.achievement) setAchievement(payload.achievement);
      toast({ title: 'AI draft populated' });
    } catch (err) {
      toast({
        title: 'AI draft fill failed',
        description: err instanceof Error ? err.message : 'An unexpected error occurred',
        variant: 'destructive',
      });
    } finally {
      setIsDraftFilling(false);
    }
  };
      ...
      <div className="bg-card border border-border rounded-2xl shadow-2xl w-full max-w-2xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border flex-none">
          <div className="flex items-center gap-2 font-semibold text-foreground">
            ...
            {initialParentProcess ? 'Create Subprocess' : 'Add New Process'}
          </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
            ...
            {initialParentProcess ? (
              <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-2 text-xs text-muted-foreground">
                This subprocess will be created under{' '}
                <span className="font-medium text-foreground">
                  {`PRO-${String(initialParentProcess.number).padStart(3, '0')} ${initialParentProcess.processName || initialParentProcess.processDescription}`}
                </span>.
                You can also create deeper subprocess levels from any subprocess row in the catalogue.
              </div>
            ) : (
              <ModalField label="Subprocess Of">
                <select value={parentProcessId} onChange={e => setParentProcessId(e.target.value)} className={inputCls}>
                  <option value="">No parent process</option>
                  {processes
                    .slice()
                    .sort((a, b) => a.number - b.number)
                    .map(p => (
                      <option key={p.id} value={String(p.id)}>
                        {`PRO-${String(p.number).padStart(3, '0')} ${p.processName || p.processDescription}`}
                      </option>
                    ))}
                </select>
              </ModalField>
          ...
          <div className="flex items-center gap-3 px-6 py-4 border-t border-border flex-none bg-card">
            ...
            <button
              type="button"
              onClick={handleDraftAiFill}
              disabled={isBusy || !processDescription.trim()}
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 border border-primary/30 bg-primary/10 hover:bg-primary/20 text-primary rounded-xl text-sm font-medium transition-colors disabled:opacity-50"
            >
              {isBusy
                ? <><Loader2 className="w-4 h-4 animate-spin" />{isCreating ? 'Saving…' : 'AI Filling…'}</>
