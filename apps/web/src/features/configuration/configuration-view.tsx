...
import { THEMES, THEME_PREVIEW, applyTheme } from '@/features/settings/settings-view';
import { BUSINESS_OS_ORG_NAME_KEY, LEGACY_NONPROFIT_OS_ORG_NAME_KEY, setStoredValue } from '@/shared/lib/storage';
  ...
  const save = async () => {
    ...
    try {
      ...
      if (r.ok) {
        ...
        setStoredValue(BUSINESS_OS_ORG_NAME_KEY, sidebarName, LEGACY_NONPROFIT_OS_ORG_NAME_KEY);
...
function ColourSchemeSection({ fetchHeaders }: { fetchHeaders: () => Record<string, string> }) {
  ...
  const saveOrg = async (themeId: string) => {
    setOrgTheme(themeId);
    if (!personalTheme) applyTheme(themeId);
    setSavingOrg(true);
    try {
      ...
      if (!personalTheme) applyTheme(orgTheme ?? 'dark');
    }
    setSavingOrg(false);
  };
